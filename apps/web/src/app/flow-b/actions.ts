...
export interface FlowBFormState {
	status: 'idle' | 'success' | 'error';
	message?: string;
}

// These two source values are stable analytics identifiers: Founder Pilot conversion rate is
// compared between source = 'flow-b-founder-pilot' and the control's (null-source) signups,
// and 'flow-b-calculator' is the one measurable secondary signal for the lost-revenue
// calculator. Do not rename either value without accounting for the impact on historical
// reporting/queries that already filter early_access_signups on `source`.
const FLOW_B_FOUNDER_PILOT_SOURCE = 'flow-b-founder-pilot';
const FLOW_B_CALCULATOR_SOURCE = 'flow-b-calculator';

export async function submitFlowBFounderSignup(
	_prevState: FlowBFormState,
	formData: FormData,
): Promise<FlowBFormState> {
	...
	const { error } = await supabase.from('early_access_signups').insert({
		...
		source: FLOW_B_FOUNDER_PILOT_SOURCE,
	});

	if (error && error.code !== '23505') {
		console.error('Failed to save flow-b founder signup', error);
...
export async function submitMissedRevenueLead(
	...
	const { error } = await supabase.from('early_access_signups').insert({
		...
		facility_name: typeof facilityName === 'string' && facilityName.trim() !== '' ? facilityName.trim() : null,
		message,
		source: FLOW_B_CALCULATOR_SOURCE,
	});
