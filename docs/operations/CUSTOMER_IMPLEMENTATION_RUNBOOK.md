...
├── name                  (required)
├── twilio_phone_number   (set in Phase 4, below)
├── vapi_assistant_id     (set in Phase 4, below)
└── workspace_type        ('demo' | 'internal' | 'customer' — defaults to 'customer';
                            see docs/architecture/WORKSPACE_ARCHITECTURE.md. Metadata only
                            as of Phase 44a — nothing in the application behaves
                            differently based on it yet, so a real onboarding doesn't
                            need to think about this beyond leaving the default alone)
```
