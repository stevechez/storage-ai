...
- **A wrong Vapi webhook secret fails completely silently, with no error visible to the founder or caller** (Phase 42, "Become Joe" dry run, real production incident, not hypothetical). If a facility's Vapi assistant is configured with a webhook secret that doesn't match Vercel's deployed `VAPI_WEBHOOK_SECRET`, the phone call itself works perfectly — Vapi answers, converses, ends normally — while every result silently vanishes; the only trace is `Rejected Vapi webhook: missing or invalid secret header` in Vercel's function logs. Took three real retry cycles to diagnose and fix during Harbor Self Storage's onboarding. Not escalated to "must resolve before first founder customer" because `FOUNDER_PROVISIONING_CHECKLIST.md`'s verification step (query the database after a real test call, before ever telling an operator they're live) is specifically designed to catch this during onboarding — but only if that step is actually followed. Full incident narrative in `docs/operations/FRICTION_LOG.md`. Candidate fix, not built: some form of webhook-delivery visibility (even just surfacing rejected-webhook counts somewhere a founder would see them), rather than requiring a dive into Vercel's raw function logs.
